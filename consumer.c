...
void consumer_update_state(int id, WorkerState state) {
	consumer_state[id] = state;
	sem_post(&ui_update);
}
...
void* consumer_run(void* arg) {
	...
	while (1) {
		consumer_update_state(id, IDLE);
		sem_wait(&stored_goods);
		
		consumer_update_state(id, TAKING_ITEM);
		nanosleep(&interaction_time, NULL);
		
		consumer_update_state(id, PROCESSING);
		randomize_proc_time(&processing_time);
		nanosleep(&processing_time, NULL);
		
		consumer_update_state(id, STORING_ITEM);
		nanosleep(&interaction_time, NULL);
